...
<body>
  ...
  <table cellspacing="0" border="1" width="80%">
    ...
    $someguide=FALSE;
    $showmore=TRUE;
    $retrieve_sutta_acronym=mysql_query("SELECT sutta_acronym, sutta_name, sutta_text_url_link FROM $table5 WHERE sutta_id='$sutta_id'");
    ...
    echo $retrieve_info['sutta_acronym']."</a> - ".$retrieve_info['sutta_name']." - ".$volpage_info;
...
//************************************************************************************************************
    ...
    {  //$result2 = mysql_query("SELECT subdivision_id, sutta_id, sutta_acronym, sutta_number, sutta_coded_name, sutta_text_url_link, volpage_info, collection_language_name, biblio_entry_id from $table5, $table10 WHERE $table5.sutta_id='$row[entry_id]' AND $table5.collection_language_id=$table10.collection_language_id");
       ...
       {  $result1 = mysql_query("SELECT subdivision_id, sutta_id, sutta_acronym, alt_sutta_acronym, sutta_number, sutta_name, sutta_text_url_link, volpage_info, collection_language_name, biblio_entry_id from $table5, $table10 WHERE $table5.sutta_id='$record[sutta_id]' AND $table5.collection_language_id=$table10.collection_language_id");
          $info1 = mysql_fetch_assoc($result1);
...
//    echo "<br>after sort<br><br>";
    ...
    {  
       ...
       {  $result5 = mysql_result(mysql_query("SELECT biblio_entry_text from $table11 WHERE biblio_entry_id='$record[biblio_entry_id]'"),0);
          ...
          echo ' '.$record[volpage_info];
          // echo '<div class="plus" id="'.$pluscount.'">'.$result5.'</div>';



          //echo ' [ <span class="sc" title="'.$result5.'">+</span> ] ';
	}
	else echo $record[volpage_info].'&nbsp;';
	if($record[footnote])
	{  $infocount++;
  ...
  <?php if($someguide) echo '* Partial parallel.<br/>';
        if($acronym_alt) echo '[...] indicates PTS numbering.';
  ?>
  </center>
